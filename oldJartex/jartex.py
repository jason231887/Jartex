...
def main():
    ...
    with open ('jartex.csv', encoding='utf-8') as csvFile:
        temp = 0
        ...
        for row in reversed(list(reader)):
            #Get the last row
            tempString = str(row)
            #Seperate the name from the purchase
            ...
            #Append the names/purchases to array
            recentName.append(tempName)
            recentItem.append(tempItem)
            #Go to next row
            temp+=1
            #Once 5 rows are added to array, move on
            if temp > 5:
                break
    ...
    f = open('jartex.csv', "a", newline = '', encoding="utf-8")
